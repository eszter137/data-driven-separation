...
class GaussianProcess(InputModel):
    ...
    def get_dataset(
        ...
        if P is None:
            raise ValueError("This class generates dataset online. Please provide P.")
...
class SpikedGaussian(InputModel):
    """
    Gaussian process with given mean and covariance matrix.

    """

    def __init__(self, spike,covariance,gain,mean=None):
        """
        Parameters:
        -----------
        covariance, mean : (D, D), (D)
        ...
        self._input_dim = covariance.shape[0]
        self._rescaled_spike = np.sqrt(gain / self._input_dim) * spike 
        self._mean = np.zeros(self._input_dim) if mean is None else mean.numpy()
        self._covariance = covariance.numpy()

    def sample(self, P=1):
        xs = torch.tensor(
            np.random.randn(P)[:, None] *self._rescaled_spike + multivariate_normal.rvs(mean=self._mean, cov=self._covariance, size=P),
            dtype=torch.float32,
        )
...
class SpikedGaussianMultiSpike(InputModel):
    """
    Gaussian process with given mean and covariance matrix.

    """

    def __init__(self, spikes,covariance,gain,mean=None):
        """
        Parameters:
        -----------
        covariance, mean : (D, D), (D)
        ...
        self._input_dim = covariance.shape[0]
        self._rescaled_spikes=[]
        for ind in range(len(spikes)):
            self._rescaled_spikes.append(np.sqrt(gain / self._input_dim)*(spikes[ind]))
        self._mean = np.zeros(self._input_dim) if mean is None else mean.numpy()
        self._covariance = covariance.numpy()

    def sample(self, P=1):
        tmp_xs = multivariate_normal.rvs(mean=self._mean, cov=self._covariance, size=P),
        for ind in range(len(self._rescaled_spikes)):
            tmp_xs+= np.random.randn(P)[:, None] *self._rescaled_spikes[ind]
        xs = torch.tensor(tmp_xs,dtype=torch.float32,)
        return xs
...
class WhitenedModel(InputModel):
    """
    ...
    """

    def __init__(self, spike, gain=1., mean=None,distr="Rademacher"):
        """
        Parameters:
        -----------
        """
        ...
        self.spike = spike 
        self._input_dim = np.shape(spike)[0]
        self.mean = np.zeros(self._input_dim) if mean is None else mean.numpy()
        self.gain=gain
        self.distr=distr
        self.S = np.eye(self._input_dim) - self.gain / self._input_dim / (1 + self.gain + np.sqrt(1 + self.gain)) * np.outer(self.spike, self.spike)


    def sample(self, P=1):
        if self.distr == "Laplace":
            gs = np.sqrt(0.5) * rnd.laplace(size=P)
        elif self.distr == "Rademacher":
            gs = np.sign(rnd.randn(P)) 
        else:
            print("Distribution is not set, using Rademacher")
            gs = np.sign(rnd.randn(P)) 
        noise = rnd.randn(P, self._input_dim)
        X = np.sqrt(self.gain / self._input_dim) * (gs[:, None] * self.spike) + noise # changed 6th March! rnd.randn(P, self._input_dim)
        X = X @ self.S
...
class WhitenedModelMultiSpike(InputModel):
    """
    ...
    """

    def __init__(self, spikes, gain=1.,distr="Rademacher", mean=None):
        """
        Parameters:
        -----------
        """
        ...
        self.spikes = spikes #np.sign(rnd.randn(self._input_dim))
        self._input_dim = np.shape(spikes[0])[0]
        self.mean = np.zeros(self._input_dim) if mean is None else mean.numpy()
        self.gain=gain
        self.distr=distr
        self.S = dict()
        for ind in range(len(self.spikes)):
            self.S[ind] =np.eye(self._input_dim) - self.gain / self._input_dim / (1 + self.gain + np.sqrt(1 + self.gain)) * np.outer(self.spikes[ind], self.spikes[ind])
    ...
    def sample(self, P=1):
        ...
        for ind in range(len(self.spikes)):
            if self.distr == "Laplace":
                ...
                gs = np.sign(rnd.randn(P)) 
            X += np.sqrt(self.gain / self._input_dim) * (gs[:, None] * self.spikes[ind])
            X = X @ self.S[ind]
        xs = torch.from_numpy(X).type(torch.float32)
...
class WhitenedModel_RefClass(InputModel):
    """
    ...
    """

    def __init__(self, input_dim,  mean=None): 
        """
        Parameters:
        -----------
        """
