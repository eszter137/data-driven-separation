...
this_root="/u/e/eszekely/"
sys.path.insert(0,"/m100/home/userexternal/eszekely/research/nn_gp_comparison/my_functions/")

import define_networks
import general_functions

sys.path.insert(0,"/m100/home/userexternal/eszekely/research/nn_gp_comparison/my_functions/data_nlgp_gp/files/")
...
parser.add_argument("--l1_width",type=int,default=256)

parser.add_argument("--input_dim",type=int,default=1)
parser.add_argument("--input_length",type=int,default=20)
parser.add_argument("--input_corr",nargs='+',type=float,default=[0.1,0.1])
parser.add_argument("--input_names",nargs='+',type=str,default=["nlgp","gp"])
parser.add_argument("--input_gain",type=float,default=3)
parser.add_argument("--input_pbc",choices=("True","False"))
...
if not (num_classes==len(input_corr)):
    print("problem with length of inputs: different length for correlation and input types")
this_num_classes=len(input_names)
gain = args.input_gain  # how non-linear are the inputs?
pbc = (args.input_pbc=="True")  # periodic boundary conditions

filename_str_dataset=("dim"+str(dim)+"D"+str(D)+"_inputs_"+general_functions.concatenate_list_to_str(input_names)+"_"
                      +general_functions.concatenate_list_to_str(input_corr)+"_gain"+str(gain)+"_pbc"+str(pbc))
...
this_lr=0.5*(this_l1_width/100.) # *(D/20.)
this_wd=(0.000005*(100./this_l1_width))
    

xs = dict()
ys_target = dict()
ys_prob = dict()
distributions = tasks.build_nlgp_mixture(input_names, input_corr, D, pbc, gain)
...
def run_nn_given_inds(net, inds_to_run_on,mode="train"):
    ...
    for train_i in inds_to_run_on:
        ...
        # we want to write out the unscaled loss to the logfile
...
net=define_networks.TwoLayerNet_NTK(x_width=(dim*D),l1_width=this_l1_width,num_classes=this_num_classes,train_layers=[not(use_RF),True],train_biases=[this_train_bias,False],alpha=alpha) #,std0=this_std)

this_fit_type_dir="2LNN_NTK"
this_fit_type_file="2LNN_NTK_A"+str(alpha)
#print("fit type is: ", this_fit_type_file)
if not np.isclose(this_wd,0.,atol=10**-10):
    str_wd="wd"+'{:.10g}'.format(this_wd) #"wd"+str(this_wd)
else:
    str_wd=""
if useLrCosine:
    tmp_file_name_root=("nlgp_gp/"+this_fit_type_dir+"/"+this_fit_type_file+filename_str_dataset
                        +"_w"+str(this_l1_width)+"bias"+str(this_train_bias)[0]+"_"+this_loss+"LrCosineAnneal"+str_wd+"Momentum"
    ...
    tmp_file_name_root=("nlgp_gp/"+this_fit_type_dir+"/"+this_fit_type_file+filename_str_dataset
                        +"_w"+str(this_l1_width)+"bias"+str(this_train_bias)[0]+"_"+this_loss+"Lr"+str(this_lr)+str_wd+"Momentum"
                        +str(this_momentum)+"Nepoch"+str(num_epoch)
                        +"train_size"+str(num_train)+"test_size"+str(num_test)+"rnd"+str(rand_seed)+"_torchrnd"+str(rand_seed_torch))

### Save train & test data
#for mode in modes: 
#    my_file_root_data=tmp_file_name_root+"_data_"+mode
#    np.savetxt(my_file_root_data+"_input.txt",xs[mode].detach().numpy())
#    np.savetxt(my_file_root_data+"_target.txt",ys_target[mode].detach().numpy())

my_logfile=tmp_file_name_root+".txt"
my_logfile_time=tmp_file_name_root+"_time.txt"
with open(my_logfile, 'w') as f_log:
    f_log.write(f"Epoch Train_loss  Test_loss  "
                +"Train_accuracy  Test_accuracy\n")
with open(my_logfile_time, 'w') as f_time:
...
for epoch in range(num_epoch): 
    ...
    for i,data in enumerate(train_data_full):
        ...
        outputs = net(inputs)
        loss = (1./alpha**2.)*criterion(outputs,labels)
    ...
    #with open(my_logfile, 'a') as f_log:
    ...
    #with open(my_logfile_time, 'a') as f_time:
    ...
    if epoch%500 == 0:
        this_file_nn=tmp_file_name_root+"_epoch"+str(epoch)+".pth"
