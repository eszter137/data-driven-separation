...
parser.add_argument("--input_pbc",choices=("True","False"),default="True")
...
filename_str_dataset=("dim"+str(dim)+"D"+str(D)+"_inputs_"+general_functions.concatenate_list_to_str(input_names)
                      +"_gain"+str(gain)+str_spike+"_pbc"+str(pbc))
...
this_lr=args.lr #0.01 #0.05
#this_std = args.l1_std
#this_lr=0.1 #*(this_l1_width/100.) # *(D/20.)
#this_wd=0.0005 #*(100./this_l1_width))
...
net=define_networks.TwoLayer(dim*D, this_l1_width,nonlinearity=non_linearity)
...
if num_spike>1:
    data_dir="whitened_model_"+this_distr+"_distr_rnd_noise_"+str(num_spike)+"spike"
else:
    data_dir="whitened_model_"+this_distr+"_distr_rnd_noise"
if useLrCosine:
...
distributions = tasks.build_whitened_mixture_multi_spike(input_names=input_names,D=D,gain=gain,num_spike=num_spike,spikes=list_spikes,distr=this_distr)
