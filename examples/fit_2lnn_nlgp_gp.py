...
root_dir ="/u/e/eszekely/"
sys.path.insert(0,root_dir+"/programs/nn/my_functions/")
sys.path.insert(0,root_dir+"/programs/nn/CIFAR10 clones/")
...
parser.add_argument("--l1_width",type=int,default=16)
...
parser.add_argument("--input_gain",type=float,default=5.)
...
this_fit_type_dir="2LNN"
this_fit_type_file="2LNN"
if not np.isclose(this_wd,0.,atol=10**-10):
    str_wd="wd"+'{:.10g}'.format(this_wd) #"wd"+str(this_wd)
else:
    str_wd=""
if useLrCosine:
    tmp_file_name_root=("nlgp_gp/"+this_fit_type_dir+"/"+filename_str_dataset
...
for epoch in range(num_epoch): 
    ...
    print(str(epoch)+f"    {results_train['loss']:.6f}    {results_test['loss']:.6f}    "
                +f"{results_train['accuracy']:.6f}    {results_test['accuracy']:.6f}\n")
